...
import { FoodItem, CategoryType, FreshnessStatus, LanguageType } from '../types';
import { t, getLocalizedCategory, getLocalizedLocation } from '../utils/i18n';
      ...
      <div className="bg-white w-full max-w-md rounded-3xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-emerald-50/50">
          <h2 className="font-bold text-slate-800 text-lg">
            {initialItem ? (language === 'fr' ? 'Modifier l\'aliment' : language === 'ar-MA' ? 'تعديل السلعة' : 'Edit Item') : t('addItem', language)}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-4 text-slate-700 text-sm">
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1">
              {language === 'fr' ? 'Nom de l\'aliment *' : language === 'ar-MA' ? 'اسم السلعة *' : 'Item Name *'}
            </label>
            <input
              type="text"
              required
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">{t('category', language)}</label>
              <select
                value={category}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">
                {language === 'fr' ? 'État de fraîcheur' : language === 'ar-MA' ? 'حالة الطراوة / الغمل' : 'Freshness Status'}
              </label>
              <select
                value={freshness}
                ...
                className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 font-medium text-xs"
              >
                <option value="fresh">🟢 {t('fresh', language)}</option>
                <option value="soon_to_expire">🟡 {t('expiringSoon', language)}</option>
                <option value="expired">🔴 {t('expired', language)}</option>
              </select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">
                {language === 'fr' ? 'Quantité' : language === 'ar-MA' ? 'الكمية' : 'Quantity'}
              </label>
              <input
                type="number"
                step="0.1"
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">
                {language === 'fr' ? 'Unité' : language === 'ar-MA' ? 'الوحدة' : 'Unit'}
              </label>
              <input
                type="text"
                value={unit}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1">{t('location', language)}</label>
            <select
              value={locationInFridge}
              onChange={(e) => setLocationInFridge(e.target.value)}
              className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 font-medium text-xs"
            >
              <option value="Crisper Drawer">🥬 {getLocalizedLocation('Crisper Drawer', language)}</option>
              <option value="Top Shelf">🥛 {getLocalizedLocation('Top Shelf', language)}</option>
              <option value="Middle Shelf">🧀 {getLocalizedLocation('Middle Shelf', language)}</option>
              <option value="Bottom Shelf">🥩 {getLocalizedLocation('Bottom Shelf', language)}</option>
              <option value="Door Shelf">🫙 {getLocalizedLocation('Door Shelf', language)}</option>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1">
              {language === 'fr' ? 'Notes / Conseils' : language === 'ar-MA' ? 'ملاحظات' : 'Notes / Tips'}
            </label>
            <textarea
              rows={2}
              value={notes}
          ...
          <div className="pt-2 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              >
                ...
                {initialItem ? (language === 'fr' ? 'Mettre à jour' : language === 'ar-MA' ? 'حفظ التعديل' : 'Update') : t('addItem', language)}
              </button>
            </div>
          </div>
        </form>

...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  const tabs = [
    ...
    { id: 'stores' as TabType, label: t('stores', lang), icon: MapPin },
  ];

  if (!visible) return null;

  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/92 backdrop-blur-xl border-t border-ink/[0.07] px-1 sm:px-3 pt-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] shadow-[0_-12px_40px_-20px_rgba(11,61,46,0.25)]">
        ...
        {tabs.map((tab) => {
          ...
          if (tab.highlight) {
            return (
              <button
                ...
                className="relative -top-4 sm:-top-5 flex min-w-0 flex-1 sm:flex-none flex-col items-center group transition-all duration-200 hover:-translate-y-1 px-1"
              >
                <div className={`w-11 h-11 sm:w-14 sm:h-14 rounded-full bg-cold text-pine-deep border-4 border-white shadow-[0_12px_30px_-10px_rgba(34,197,94,0.8)] flex items-center justify-center transition-all duration-200 hover:scale-110 active:scale-90 ${
                  isActive ? 'ring-4 ring-cold/25 bg-cold-dark text-white' : ''
                }`}>
                  <Icon className="w-5 h-5 sm:w-6 sm:h-6 stroke-[2.5]" />
                </div>
                <span className={`text-[7.5px] sm:text-[9px] font-bold uppercase tracking-wide sm:tracking-widest whitespace-nowrap mt-1 transition-colors ${
          ...
          return (
            <button
              key={tab.id}
              onClick={() => onNavigate(tab.id)}
              className={`relative min-w-0 flex flex-1 sm:flex-none flex-col items-center py-1 px-1 sm:px-2 transition-all duration-200 hover:scale-110 active:scale-90 ${
                isActive ? 'text-cold-dark' : 'text-ink/30 hover:text-ink/60'
              }`}
            >
              <div className="relative">
                <Icon className={`w-[19px] h-[19px] sm:w-5 sm:h-5 transition-transform duration-200 ${isActive ? 'stroke-[2.5]' : 'stroke-[1.8]'}`} />

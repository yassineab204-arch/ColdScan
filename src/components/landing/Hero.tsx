      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 pt-16 sm:pt-24 lg:pt-28 pb-16 sm:pb-24">
        <div className="grid items-center gap-14 lg:grid-cols-[1.05fr_0.95fr] lg:gap-8">
          <div className="max-w-xl mx-auto lg:mx-0 text-center lg:text-left">
            ...
            >
              ...
              <span className="relative whitespace-nowrap">
                {t('heroTitle2', lang)}
                <svg className="absolute -bottom-2 left-0 w-full text-cold" viewBox="0 0 220 12" fill="none" aria-hidden="true">
                  <path d="M3 9C60 3 160 3 217 9" stroke="currentColor" strokeWidth="5" strokeLinecap="round" />
                </svg>
              </span>
            </motion.h1>
            ...
            >
              <button onClick={() => onNavigate('scan')} className="group inline-flex w-full sm:w-auto items-center justify-center gap-2.5 rounded-full bg-cold px-7 py-4 text-[15px] font-bold text-pine-deep shadow-[0_16px_40px_-12px_rgba(34,197,94,0.7)] transition-all duration-200 hover:bg-cold/90 hover:shadow-[0_20px_48px_-12px_rgba(34,197,94,0.8)] hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98]">
                <Camera className="w-5 h-5" strokeWidth={2.4} />
                {t('heroScanFridge', lang)}
              </button>
              <button onClick={() => onNavigate('recipes')} className="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-full bg-white px-7 py-4 text-[15px] font-bold text-pine ring-1 ring-ink/10 shadow-sm transition-all duration-200 hover:ring-cold/60 hover:text-pine-light hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98]">
                <ChefHat className="w-5 h-5" strokeWidth={2.2} />
                {t('heroExploreRecipes', lang)}
              </button>
            </motion.div>
            ...
            >
              <div className="text-center lg:text-left">
                ...
                <p className="mt-1 text-[10px] font-semibold uppercase tracking-widest text-ink/50">{t('heroItemsInFridge', lang)}</p>
              </div>
              <div className="h-8 w-px bg-ink/10" />
              <div className="text-center lg:text-left">
                <p className="text-2xl font-extrabold text-pine leading-none">{recipeCount}</p>
                <p className="mt-1 text-[10px] font-semibold uppercase tracking-widest text-ink/50">{t('heroRecipesReady', lang)}</p>
              </div>
              <div className="h-8 w-px bg-ink/10" />
              <div className="text-center lg:text-left">
                <p className="text-2xl font-extrabold text-cold-dark leading-none">{expiringCount}</p>
                <p className="mt-1 text-[10px] font-semibold uppercase tracking-widest text-ink/50">{t('heroUseSoon', lang)}</p>
              </div>
            </motion.div>
          </div>
          ...
          <motion.div
            ...
            className="relative"
          >
            <FridgeVisual itemsDetected={itemsDetected} expiringCount={expiringCount} lang={lang} />
            <div className="mt-10 flex items-center justify-center gap-2.5 text-[10px] font-bold uppercase tracking-[0.18em] text-ink/45">
              <span>{t('heroCaptionFridge', lang)}</span>
              <Arrow />
              <span className="flex items-center gap-1 text-cold-dark"><Camera className="w-3.5 h-3.5" /> {t('heroCaptionScan', lang)}</span>
              <Arrow />
              <span className="flex items-center gap-1 text-cold-dark">{t('heroCaptionAI', lang)}</span>
              <Arrow />
              <span className="flex items-center gap-1 text-cold-dark"><ShoppingBag className="w-3.5 h-3.5" /> {t('heroCaptionRecipes', lang)}</span>
            </div>
          </motion.div>
        </div>
      </div>
